# Environment Manager -- Cadence Plugin
...
Copy or symlink this folder into `dashboard/plugins/` in your Cadence installation:

```
dashboard/plugins/env-manager/
```

Restart Cadence. The plugin will appear as a new tab.

## Configuration

Settings are available in Cadence Settings > Plugins > Environment Manager:
...
The plugin uses `getConfig().Repos` from the Cadence API to discover configured repos and their local paths. For each repo it:
